...
function timelineDotClass(type: AuditSourceType) {
  if (type === "original") return "border-blue-200 bg-blue-400 shadow-[0_0_18px_rgba(96,165,250,0.55)]";
  if (type === "extracted") return "border-emerald-200 bg-emerald-400 shadow-[0_0_18px_rgba(52,211,153,0.45)]";
  if (type === "inferred") return "border-violet-200 bg-violet-400 shadow-[0_0_18px_rgba(167,139,250,0.45)]";
  if (type === "manual_required") return "border-amber-200 bg-amber-400 shadow-[0_0_18px_rgba(251,191,36,0.45)]";
...
function buildComplianceDimensions(view: AuditView): ComplianceDimension[] {
  ...
  const missingCount = view.material_scan.filter((item) => item.status === "missing").length;
  const hasTimelineMissing = view.timeline.some((item) => item.source_type === "missing");
  return [
    { label: "使用范围", value: resultText(scope), tone: resultTone(scope?.result) },
    { label: "流程合规", value: resultText(process), tone: resultTone(process?.result) },
    { label: "时序完整", value: hasTimelineMissing ? "缺失" : "合规", tone: hasTimelineMissing ? "rose" : "emerald" },
    { label: "材料完整", value: missingCount > 0 ? "资料不完整" : "合规", tone: missingCount > 0 ? "amber" : "emerald" },
    { label: "金额复核", value: resultText(amount), tone: resultTone(amount?.result) },
  ];
}

function buildIssueCards(view: AuditView): IssueCard[] {
  const issues: IssueCard[] = [];
  const missingItems = view.material_scan.filter((item) => item.status === "missing");
  const timelineWarnings = view.timeline.filter((item) => item.warning || item.source_type === "missing");
  const scope = findAuditCard(view, "使用范围");
  const process = findAuditCard(view, "流程");
  const amount = findAuditCard(view, "金额");
  ...
  if (timelineWarnings.length > 0) {
    issues.push({
      code: "TIMING",
      title: "施工与决议时序需复核",
      level: timelineWarnings.some((item) => item.source_type === "missing") ? "高风险" : "需复核",
      summary: timelineWarnings.map((item) => `${item.label}：${item.warning || item.business_meaning}`).join("；"),
  ...
  if (process && process.result !== "compliant") {
    issues.push({
      ...
      recommendation: process.recommendation || "请结合表决、公告、公示和验收材料复核流程闭环。",
      tone: "amber",
    });
  }
  ...
  if (amount && amount.result !== "compliant") {
    issues.push({
      ...
      recommendation: amount.recommendation || "请结合审价合同、审价报告和最终结算金额复核。",
      tone: "indigo",
    });
  }
  ...
  if (scope) {
    issues.push({
      ...
      recommendation: scope.recommendation || "请结合原始材料和法规命中结果保留复核记录。",
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${toneBorderClass(conclusion.risk_level === "high" ? "rose" : conclusion.risk_level === "low" ? "emerald" : "amber")}`}>
        ...
        <button type="button" className="inline-flex w-fit shrink-0 items-center justify-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-500/10 px-4 py-3 text-xs font-semibold text-cyan-100 transition hover:border-cyan-300/40 hover:bg-cyan-500/15">
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 xl:col-span-2">
      <div className="flex items-start justify-between gap-3">
        <p className="text-xs font-semibold text-slate-400">{field.field_label}</p>
      ...
      <p className="mt-3 break-words text-base font-semibold leading-relaxed text-slate-100">{field.display_value}</p>
      <p className="mt-3 break-words text-[11px] text-slate-500">来源：{field.source_label || "未识别"}</p>
...
function OverviewMetricCard({ field }: { field: AuditViewField }) {
  return (
    <div className="rounded-2xl border border-indigo-300/15 bg-indigo-500/[0.06] p-4">
      <div className="flex items-start justify-between gap-3">
        <p className="text-xs font-semibold text-indigo-200/80">{field.field_label}</p>
        <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${sourceBadgeClass(field.source_type)}`}>
          {sourceLabel(field.source_type)}
        </span>
      </div>
      <p className="mt-3 break-words text-xl font-black text-white">{field.display_value}</p>
      <p className="mt-3 break-words text-[11px] text-slate-500">来源：{field.source_label || "未识别"}</p>
      <p className="mt-1 text-[10px] text-slate-500">置信度：{Math.round((field.confidence || 0) * 100)}%</p>
    </div>
  );
...
export function ProjectOverviewPanel({ view }: { view: AuditView }) {
  const textFields = ["project_name", "repair_scope", "repair_reason", "applicant"].map((key) => view.project_overview[key]).filter(Boolean);
  const metricFields = ["budget_amount", "final_amount", "need_cost_review", "repair_nature"].map((key) => view.project_overview[key]).filter(Boolean);
  return (
    <section className="glass-card p-6">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-4">
        {metricFields.map((field) => (
          <OverviewMetricCard key={field.field_key || field.field_label} field={field} />
        ))}
      </div>
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-4">
        {textFields.map((field) => (
          <OverviewTextCard key={field.field_key || field.field_label} field={field} />
        ))}
      </div>
    </section>
  );
...
export function IssueCardsPanel({ view }: { view: AuditView }) {
  ...
  return (
    <section className="grid grid-cols-1 gap-4 xl:grid-cols-3">
      {issues.map((issue) => (
        <article key={issue.code} className={`glass-card border-l-4 p-5 ${issueAccentClass(issue.tone)}`}>
          <div className="flex items-start justify-between gap-4">
            <span className={`rounded-full border px-2.5 py-1 text-[10px] font-black tracking-[0.18em] ${toneBorderClass(issue.tone)}`}>
              {issue.code}
            </span>
            <div className={`rounded-xl border p-2 ${toneBorderClass(issue.tone)}`}>
          ...
          <p className="mt-3 text-xs leading-relaxed text-slate-400">{issue.summary}</p>
          <p className="mt-3 text-xs leading-relaxed text-indigo-200">建议：{issue.recommendation}</p>
        </article>
      ))}
    </section>
  );
    ...
    <section className="glass-card h-full w-full p-6">
      ...
      <div className="relative ml-2 space-y-5 pl-8 before:absolute before:bottom-2 before:left-[7px] before:top-2 before:w-px before:bg-white/10">
        {view.timeline.map((item) => (
          <div key={`${item.label}-${item.source_label}`} className="relative rounded-xl border border-white/10 bg-white/[0.03] p-4">
            <span className={`absolute -left-[34px] top-5 h-4 w-4 rounded-full border-2 ${timelineDotClass(item.source_type)}`} />
    ...
    <section className="glass-card p-6">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">保留审计类型、判断摘要和建议动作</p>
        </div>
      </div>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {view.audit_cards.map((card) => (
          <article key={card.audit_type} className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
            <div className="flex items-start justify-between gap-4">
    ...
    <details className="glass-card group p-6">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-white">技术诊断 / 审计留痕</h3>
            <p className="text-xs text-slate-500">展开后查看系统状态、冲突数量和运行提醒</p>
          </div>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 text-xs md:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <p className="mt-2 break-words text-slate-500">LLM：{String(view.auditor_notes.llm_status?.selected_model || "未记录")}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          <p className="font-semibold text-slate-200">提醒</p>
          <ul className="mt-2 space-y-1 text-slate-500">
            {view.auditor_notes.warnings.length === 0 && <li>暂无额外提醒。</li>}
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 md:col-span-2">
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
            {view.audit_cards.map((card) => (
              <div key={card.audit_type} className="rounded-lg border border-white/5 bg-black/10 p-3">
                <div className="flex items-start justify-between gap-3">
                  <p className="font-semibold text-slate-200">{card.audit_type}</p>
                  <span className="shrink-0 rounded-full border border-white/10 px-2 py-0.5 text-[10px] text-slate-400">{card.result_label}</span>
                </div>
                <p className="mt-2 line-clamp-2 text-slate-500">{card.summary}</p>
              </div>
            ))}
          </div>
        </div>
